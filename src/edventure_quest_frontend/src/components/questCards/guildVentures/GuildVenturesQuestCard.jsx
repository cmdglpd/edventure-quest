...
export default function GuildVenturesQuestCard({ reward, title, coverImg, type, description }) {
    ...
    return <div className="card-theme-light flex flex-col gap-3 p-4">
        ...
        <div className="flex flex-col px-2">
            ...
            <Typography variant="body2" sx={{ mt: "1rem", color: colors.tertiary, display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 3, overflow: 'hidden', textOverflow: 'ellipsis' }}>{description}</Typography>
        </div>
        <Divider sx={{ backgroundColor: colors.secondary }} />
        <Button variant="contained" color="brownLight" size="small" sx={{ flexGrow: 1, fontWeight: "bold", color: colors.tertiary, borderRadius: "10px" }}>View Quests</Button>
    </div>
}
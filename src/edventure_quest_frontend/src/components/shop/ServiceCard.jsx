...
export default function ServiceCard({ name, edventurePoints, serviceImg, description }) {
    const [open, setOpen] = useState(false)

    return <>
        <ServiceDialog open={open} handleClose={() => setOpen(false)} name={name} serviceImg={serviceImg} description={description} edventurePoints={edventurePoints} />

        <div className="card-theme-light flex gap-4">
            <div className="w-[10rem]">
                <img src={serviceImg} alt="service image" style={{ width: "100%", height: "6rem", objectFit: "cover", objectPosition: "center", borderRadius: "10px", border: `4px solid ${colors.secondary}`, boxShadow: "0px 2px 4px rgba(0, 0, 0, 0.5)" }} />
            </div>
